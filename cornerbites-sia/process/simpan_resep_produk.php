...
function calculateHPPForProduct($conn, $product_id, $production_yield = 1, $production_time_hours = 1) {
    ...
    ");
    $stmtRecipes->execute([$product_id, $user_id]);
    ...
    $stmtManualLabor = $conn->prepare("
        ...
        WHERE plm.product_id = ? AND plm.user_id = ? AND lc.is_active = 1
    ");
    $stmtManualLabor->execute([$product_id, $user_id]);
    ...
    $stmtManualOverhead = $conn->prepare("
        ...
        WHERE pom.product_id = ? AND pom.user_id = ? AND oc.is_active = 1
    ");
    $stmtManualOverhead->execute([$product_id, $user_id]);
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        if ($action === 'delete' && $id && $product_id) {
            $stmt = $conn->prepare("DELETE FROM product_recipes WHERE id = ? AND product_id = ? AND user_id = ?");
            $stmt->execute([$id, $product_id, $user_id]);

            // Auto-update HPP setelah hapus item
            $productStmt = $conn->prepare("SELECT production_yield, production_time_hours FROM products WHERE id = ? AND user_id = ?");
            $productStmt->execute([$product_id, $user_id]);
            $product = $productStmt->fetch(PDO::FETCH_ASSOC);

            if ($product) {
                $hppPerUnit = calculateHPPForProduct($conn, $product_id, $product['production_yield'], $product['production_time_hours']);
                $updateStmt = $conn->prepare("UPDATE products SET cost_price = ? WHERE id = ? AND user_id = ?");
                $updateStmt->execute([$hppPerUnit, $product_id, $user_id]);
            }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        switch ($action) {
            ...
            case 'add_kemasan':
                ...
                $checkStmt = $conn->prepare("SELECT id FROM product_recipes WHERE product_id = ? AND raw_material_id = ? AND user_id = ?");
                $checkStmt->execute([$product_id, $raw_material_id, $user_id]);
                ...
                $stmt->execute([$product_id, $raw_material_id, $quantity_used, $unit_measurement, $user_id]);
            ...
            case 'edit':
                ...
                $currentStmt = $conn->prepare("SELECT raw_material_id, quantity_used FROM product_recipes WHERE id = ? AND product_id = ? AND user_id = ?");
                $currentStmt->execute([$recipe_id, $product_id, $user_id]);
                ...
                if ($currentRecipe['raw_material_id'] != $raw_material_id) {
                    $checkStmt = $conn->prepare("SELECT id FROM product_recipes WHERE product_id = ? AND raw_material_id = ? AND id != ? AND user_id = ?");
                    $checkStmt->execute([$product_id, $raw_material_id, $recipe_id, $user_id]);
                    if ($checkStmt->fetchColumn()) {
                        throw new Exception('Bahan/kemasan ini sudah ada dalam resep. Pilih bahan/kemasan yang berbeda.');
                    }
                }

                $stmt = $conn->prepare("UPDATE product_recipes SET raw_material_id = ?, quantity_used = ?, unit_measurement = ? WHERE id = ? AND product_id = ? AND user_id = ?");
                $stmt->execute([$raw_material_id, $quantity_used, $unit_measurement, $recipe_id, $product_id, $user_id]);
            ...
            case 'add_manual_overhead':
                ...
                $checkStmt = $conn->prepare("SELECT id FROM product_overhead_manual WHERE product_id = ? AND overhead_id = ? AND user_id = ?");
                $checkStmt->execute([$product_id, $overhead_id, $user_id]);
                ...
                $stmt->execute([$product_id, $overhead_id, $overhead['amount'], $finalAmount, $user_id]);
            ...
            case 'add_manual_labor':
                ...
                $checkStmt = $conn->prepare("SELECT id FROM product_labor_manual WHERE product_id = ? AND labor_id = ? AND user_id = ?");
                $checkStmt->execute([$product_id, $labor_id, $user_id]);
                ...
                $productStmt = $conn->prepare("SELECT production_time_hours FROM products WHERE id = ? AND user_id = ?");
                $productStmt->execute([$product_id, $user_id]);
                $product = $productStmt->fetch(PDO::FETCH_ASSOC);
                ...
                $stmt->execute([$product_id, $labor_id, $labor['hourly_rate'], $productionTimeHours, $totalCost, $user_id]);
            ...
            case 'delete_recipe':
                ...
                $stmt = $conn->prepare("DELETE FROM product_recipes WHERE id = ? AND product_id = ? AND user_id = ?");
                $stmt->execute([$recipe_id, $product_id, $user_id]);
            ...
            case 'update_product_info':
                ...
                $stmt = $conn->prepare("UPDATE products SET production_yield = ?, production_time_hours = ?, sale_price = ?, cost_price = ? WHERE id = ? AND user_id = ?");
                $stmt->execute([$production_yield, $production_time_hours, $sale_price, $hppPerUnit, $product_id, $user_id]);
            ...
            case 'delete_manual_overhead':
                ...
                $stmt = $conn->prepare("DELETE FROM product_overhead_manual WHERE id = ? AND product_id = ? AND user_id = ?");
                $stmt->execute([$overhead_manual_id, $product_id, $user_id]);
            ...
            case 'delete_manual_labor':
                ...
                $stmt = $conn->prepare("DELETE FROM product_labor_manual WHERE id = ? AND product_id = ? AND user_id = ?");
                $stmt->execute([$labor_manual_id, $product_id, $user_id]);
